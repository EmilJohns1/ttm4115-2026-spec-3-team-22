...
| Checkout (`app/products/[productId]/checkout.tsx`)     | Submit delivery address and pay                        | productId + per-order deliveryAddress, idempotency header | pending order + Stripe PaymentIntent client secret              | `POST /orders`                                                  |
...
2. Backend creates `pending_payment` order and Stripe PaymentIntent.
3. App confirms payment in Stripe sheet using returned client secret.
4. Stripe webhook confirms payment.
5. Backend transitions order to `confirmed`, then dispatches drone command.
...
3. Create-order endpoint + Stripe webhook
